...
int ImTextCharFromUtf8(unsigned int* out_char, const char* tranext, const char* tranext_end)
{
    unsigned int c = (unsigned int)-1;
    const unsigned char* str = (const unsigned char*)tranext;
    if (!(*str & 0x80))
    {
        c = (unsigned int)(*str++);
        *out_char = c;
    ...
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (tranext_end && tranext_end - (const char*)str < 2) return 1;
    ...
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (tranext_end && tranext_end - (const char*)str < 3) return 1;
    ...
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (tranext_end && tranext_end - (const char*)str < 4) return 1;
...
int ImTextStrFromUtf8(ImWchar* buf, int buf_size, const char* tranext, const char* tranext_end, const char** tranext_remaining)
{
    ImWchar* buf_out = buf;
    ImWchar* buf_end = buf + buf_size;
    while (buf_out < buf_end-1 && (!tranext_end || tranext < tranext_end) && *tranext)
    {
        unsigned int c;
        tranext += ImTextCharFromUtf8(&c, tranext, tranext_end);
        if (c == 0)
            break;
    ...
    *buf_out = 0;
    if (tranext_remaining)
        *tranext_remaining = tranext;
    return (int)(buf_out - buf);
}

int ImTextCountCharsFromUtf8(const char* tranext, const char* tranext_end)
{
    int char_count = 0;
    while ((!tranext_end || tranext < tranext_end) && *tranext)
    {
        unsigned int c;
        tranext += ImTextCharFromUtf8(&c, tranext, tranext_end);
        if (c == 0)
...
int ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* tranext, const ImWchar* tranext_end)
{
    char* buf_out = buf;
    const char* buf_end = buf + buf_size;
    while (buf_out < buf_end-1 && (!tranext_end || tranext < tranext_end) && *tranext)
    {
        unsigned int c = (unsigned int)(*tranext++);
        if (c < 0x80)
            *buf_out++ = (char)c;
        else
...
int ImTextCountUtf8BytesFromStr(const ImWchar* tranext, const ImWchar* tranext_end)
{
    int bytes_count = 0;
    while ((!tranext_end || tranext < tranext_end) && *tranext)
    {
...
static bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags)
{
    ...
    {
        // Different sign
        const FLOATTYPE linear_dist_mtrano_0 = ImPow(v_min >= 0 ? (FLOATTYPE)v_min : -(FLOATTYPE)v_min, (FLOATTYPE)1.0f/power);
        const FLOATTYPE linear_dist_max_to_0 = ImPow(v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max, (FLOATTYPE)1.0f/power);
        linear_zero_pos = (float)(linear_dist_mtrano_0 / (linear_dist_mtrano_0 + linear_dist_max_to_0));
    }
    else
    {
        // Same sign
